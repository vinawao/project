...
PROXY_PREFIX = "https://myproxy.app/"
...
def generate_m3u_content(channels_data):
    ...
    for ch in channels_data:
        lines.append(f'#EXTINF:-1 tvg-id="{ch["name"]}" group-title="RCTIPlus" tvg-logo="{ch["logo"]}",{ch["name"]}')
        lines.append(f'#EXTVLCOPT:http-referrer={BASE_URL}/')
        lines.append(f'#EXTVLCOPT:http-user-agent={USER_AGENT}')
        
        # tambahkan proxy di depan stream_url
        proxied_url = f'{PROXY_PREFIX}{ch["stream_url"]}'
